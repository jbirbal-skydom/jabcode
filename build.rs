...
fn main() {
    ...
    println!(
        "cargo:warning=Running build script for profile: {}",
        profile
    );
    writeln!(file, "Running build script for profile: {}", profile)
        .expect("Failed to write to log file"); // Embedding the build date
    ...
    writeln!(file, "GIT_HASH: {}", git_hash).expect("Failed to write to log file");
